...
if has('gui_running') || has('termguicolors') || &t_Co == 88 || &t_Co == 256
  ...
  " let s:syntax_fold_bg = s:mono_1
  let s:syntax_fold_bg = s:syntax_bg
  "}}}

  " Vim editor color --------------------------------------------------------{{{
  call <sid>X('Normal',       s:syntax_fg,     s:syntax_bg,      '')
  call <sid>X('EndOfBuffer',  s:syntax_bg,     s:syntax_bg,      '')
  ...
  call <sid>X('JavaScriptArrowFunction',        s:hue_3,   '', '')
  call <sid>X('JavaScriptBraces',               s:mono_2,  '', '')
  call <sid>X('JavaScriptClassBraces',          s:mono_2,  '', '')
  call <sid>X('JavaScriptClassKeywords',        s:hue_3,   '', '')
  call <sid>X('JavaScriptDocParam',             s:hue_2,   '', '')
  call <sid>X('JavaScriptDocTags',              s:hue_3,   '', '')
  call <sid>X('JavaScriptFuncBraces',           s:mono_2,  '', '')
  call <sid>X('JavaScriptFuncCall',             s:hue_2,   '', '')
  call <sid>X('JavaScriptFuncParens',           s:mono_2,  '', '')
  call <sid>X('JavaScriptFunction',             s:hue_3,   '', '')
  call <sid>X('JavaScriptGlobalObjects',        s:hue_5, '', '')
  call <sid>X('JavaScriptModuleWords',          s:hue_3,   '', '')
  call <sid>X('JavaScriptModules',              s:hue_3,   '', '')
  call <sid>X('JavaScriptNoise',                s:mono_2,  '', '')
  call <sid>X('JavaScriptNull',                 s:hue_6,   '', '')
  call <sid>X('JavaScriptOperator',             s:hue_2,   '', '')
  ...
  call <sid>X('JavaScriptObjectKey',            s:hue_1,   '', '')
  call <sid>X('JavaScriptReturn',               s:hue_3,   '', '')
  ...
  call <sid>X('JavascriptIdentifier',   s:hue_6,   '', '')
